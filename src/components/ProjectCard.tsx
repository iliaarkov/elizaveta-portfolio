...
export const ProjectCard = ({ project, lang }: Props) => {
  ...
  return (
    <motion.div 
      ...
      // hover:z-20 гарантирует, что при наведении карточка будет выше соседей,
      // но всё равно ниже Navbar (у которого z-50)
    ...
    >
      {/* Обертка с фиксированным соотношением сторон 9:16 */}
      <div className="relative w-full aspect-[9/16] overflow-hidden rounded-[2rem] bg-slate-200">
        <img 
          src={project.thumbnail} 
          alt={project.client} 
          // object-cover и h-full заставляют картинку заполнить всё пространство 9:16
          className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
          onError={(e) => {
            // Если картинка не загрузилась, покажем заглушку цвета maize
            (e.target as HTMLImageElement).src = "https://placehold.co/600x1067/FCC74B/white?text=Video+Preview";
          }}
        />
        
        {/* Градиент и текст при наведении */}
      ...
      <div className="mt-5 px-2 flex justify-between items-center">
        ...
        <div className="bg-beige p-3 rounded-full group-hover:bg-maize transition-colors shadow-sm">
